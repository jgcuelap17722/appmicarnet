  ...
  return new Promise((resolve, error) => {
    pdf_parse(readPdf)
      .then(async function (data) {
        ...
        dataPdf.nacionalidad = 'PERÚ' /* await helpers.betweenText(
          "Nacionalidad / Nationality ",
          "Sexo / Sex  ",
          "\n",
          data.text
        ...
        dataPdf.tipoVacuna = 'Vacuna contra Covid';
        /* await helpers.betweenText(
          "Vacuna / Vaccine ",
          "Vacunado / Vaccinated",
          "\n",
          data.text
        ...
        dataPdf.carnet = {
          ...
          separator: obj.carnet.separator,
        };
...
helpers.calcularEdad = (time) => {
  let MILLISECONDS_IN_A_YEAR = 1000 * 60 * 60 * 24 * 365;
  let converTime = time.toISOString().substring(0, 10);
  let date_array = converTime.split('-')
